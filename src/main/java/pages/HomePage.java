...
public class HomePage {
	...
	// use of navigate()
	// mostly important interview question, never used in framework or in real time
	// environment
	...
	public void use_of_mouse_hoverAction_on_ourLocations() {
		...
		WebElement ourLocations = driver
		...
		driver.findElement(By.xpath("//a[text()='The Mount Sinai Hospital']//parent::li[@class='notranslate']"))
				.click();
	...
	// alternate of click()
	// very very Important interview question + they ask you to write the code in MS
	// word or verbally
	// JavaScriptExecutor is an Interface that helps to execute JavaScript through
	// Selenium Webdriver.
	// so, practice it by paper pen, then in ms word
	// when you see error message "click intercepted" or
	// "ElementNotInteractableException", you use it
	// login button used
	public void alternate_of_click_method() {
		WebElement loginBttn = driver.findElement(By.xpath("//button[contains(@id, 'submit')]"));
		// in the above line we created the webElement, WE can create like
		// @FindBy(xpath="//button[contains(@id, 'submit')]") WebElement loginBttn.
	...
	// interview question
	// how to input text inside a field by JavascriptExecutor, alternate of
	// sendKeys()
	...
	// TODO: Nasir Fail, why? Test case passed but show failed after clicking
	// button, may be the executescript lost the memory of putting value in fields
	...
	public void how_to_handle_hidden_element_by_regular_selenium_method() {
		...
		// The search field will be disappeared, but we can pass value on it, as we got
		// the info before
		...
		// whenever you find element not interactable in console, that is for sure a
		// hidden element
	}
	...
	public void how_to_handle_hidden_element_by_javascriptExecutor_02() {
		...
		// Extra Code TODO
		// save below code for getTitle, getText of what written in field, Refresh by
		// JavscriptExecutor
		...
		String sTitle = (String) js.executeScript("return document.title;"); // .toString(); // fetching page title by
																				// javascript
	...
	public void first_name_validation() {
		...
		pause(3000);
		WebElement alphabeticCharactersErrorMessageUnderTheField = driver
				.findElement(By.xpath("//small[text()='Must be alphabetic characters.']"));
		...
		WebElement fNameRequiredFiledErrorMesssage = driver
				.findElement(By.xpath("//small[text()='First Name is a required field.']"));
		pause(3000);
	...
	// Try to remember this above 3 conditions-name for interview, most of the time
	// they asked it
	...
	public void use_of_explicitly_wait_05() {
		pause(3000);
		boolean outcome = wait.until(ExpectedConditions.textToBePresentInElement(loginButton, "Login"));// <-- This line
																										// says true or
																										// false
	...
	public void use_of_right_click_action() {
		...
		// We give Timeout to Load since its navigating to different site and can't pull
		// from base class
		driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(20));
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(20));
		...
		// line 775 is from my collection.
	...
	public void use_of_slider_action() {
		...
		// learn from here, 50 is in pixel which might not match with real volume
		// change, real volume 60
		pause(5000);
		// slider.click();
	...
	public void read_table() {
		...
		actions.keyDown(Keys.CONTROL).sendKeys(Keys.END).perform();
		pause(5000);
		// The below line will not work for row and cell, we use line 1043 by
		// cssSelector
		// WebElement table = driver.findElement(By.tagName("table")); // tag as a
		// locator
	...
	public void read_any_cell_of_a_row_of_the_table() {
		...
		WebElement cell = driver
				.findElement(By.cssSelector("table.navFooterMoreOnAmazon tr:nth-child(1) td:nth-child(7)"));
	...
	// We are doing raw way. You can use Common Actions too for enabled.
	...
	public void skipHomePageTitle02() {
		String expected = "SMS Enterprise Portal"; // title will not match
		if (expected.equals(driver.getTitle())) {
			throw new SkipException("Skipping -- as the title matches as expected");
		} else {
			System.out.println("Home Page Title doesn't match...");
	...
	public void logoDisplayed03() {
		elementDisplayed(logo); // Actual outcome from selenium method
		Assert.assertTrue(false, "Expected vs actual doesn't match"); // Expected
		// outcome
		// Assert.assertTrue(true, "Application Logo is not displayed"); // This error
		// message will appear if failed
